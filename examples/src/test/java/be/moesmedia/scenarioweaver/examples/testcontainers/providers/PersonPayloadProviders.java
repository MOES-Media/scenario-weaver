...
@TestConfiguration
public class PersonPayloadProviders {

    @Bean
    PayloadProvider<PersonScenarioContext> defaultPersonPayloadProvider() {
        return ctx -> ctx.payload(new Person(null, "Joske", "Vermeulen"));
    }

    @Bean
    PayloadProvider<PersonScenarioContext> noFirstNamePersonPayloadProvider() {
        return ctx -> ctx.payload(new Person(null, null, "Smith"));
    }
}
